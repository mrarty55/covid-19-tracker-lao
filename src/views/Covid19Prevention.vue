<template>
  <v-container>
    <v-col class="text-center"
      ><h2>ວີທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ</h2></v-col
    >
    <v-col>
      <h3>ລ້າງມືເປັນປະຈໍາ</h3>
      <p>
        ລ້າງມືເປັນປະຈໍາດ້ວຍສະບູ ຫຼືນໍ້າຢາລ້າງມືທີ່ມີສ່ວນປະສົມຂອງທາດເຫຼົ້າ
        (Alcohol) ຈະຊ່ວຍກໍາຈັດເຊື້ອອອກຈາກມືຂອງເຮົາໄດ້.
      </p>
      <h3>ເພີ່ມໄລຍະຫ່າງທາງສັງຄົມ (Social Distancing)</h3>
      <p>ຮັກສາໄລຍະຫ່າງຈາກຜູ້ຄົນຢ່າງໜ້ອຍ 1 ແມັດ</p>
      <h3>ຫຼີກລ່ຽງການສໍາຜັດສ່ວນຕ່າງໆຂອງໃບໜ້າ ເຊັ່ນ ຕາ, ດັງ</h3>
      <p>ການກະທໍາດັ່ງກ່າວອາດເຮັດໃຫ້ເຊື້ອສາມາດເຂົ້າສູ່ຮ່າງກາຍເຮົາໄດ້</p>
      <h3>ໄອຈາມໃຫ້ຖືກວິທີ</h3>
      <p>ເອົາແຂນສອກ ຫຼືດຶງຄໍເສື້ອຂຶ້ນມາປົກປິດໃບໜ້າເມື່ອໄອ ຫຼືຈາມ</p>
      <h3>
        ຖ້າຮູ້ສຶກວ່າຕົນເອງມີອາການຜິດປົກກະຕິເຊັ່ນ: ບໍ່ສະບາຍ, ເປັນໄຂ້,
        ຫາຍໃຈລໍາບາກໃຫ້ໄປພົບແພດທັນທີ
      </h3></v-col
    >
  </v-container>
</template>
<script>
export default {
  name: "Covid19Prevention",
  metaInfo: {
    title: "ວິທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ",
    meta: [
      {
        name: "title",
        vmid: "title",
        content: "ວິທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ"
      },
      {
        name: "description",
        vmid: "description",
        content: "ວີທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ"
      },
      {
        name: "og:type",
        vmid: "og:type",
        property: "og:type",
        content: "website"
      },
      {
        name: "og:title",
        vmid: "og:title",
        property: "og:title",
        content:
          "ວິທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ - ເວັບໄຊຕິດຕາມການແພ່ລະບາດຂອງ COVID-19"
      },
      {
        name: "og:description",
        vmid: "og:description",
        property: "og:description",
        content: "ວິທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ"
      },
      {
        name: "og:url",
        vmid: "og:url",
        property: "og:url",
        content: "https://covid19.artyboy.dev/covid-19-prevention"
      },
      {
        name: "org:image",
        vmid: "org:image",
        property: "org:image",
        content: "https://covid19.artyboy.dev/covid19.webp"
      },
      {
        name: "twitter:card",
        vmid: "twitter:card",
        property: "twitter:card",
        content: "summary"
      },
      {
        name: "twitter:title",
        vmid: "twitter:title",
        property: "twitter:title",
        content:
          "ວິທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ - ເວັບໄຊຕິດຕາມການແພ່ລະບາດຂອງ COVID-19"
      },
      {
        name: "twitter:description",
        vmid: "twitter:description",
        property: "twitter:description",
        content: "ວິທີການປ້ອງກັນ COVID-19 ເບື້ອງຕົ້ນ"
      },
      {
        name: "twitter:url",
        vmid: "twitter:url",
        property: "twitter:url",
        content: "https://covid19.artyboy.dev/covid-19-prevention"
      },
      {
        name: "twitter:image",
        vmid: "twitter:image",
        property: "twitter:image",
        content: "https://covid19.artyboy.dev/covid19.webp"
      }
    ]
  }
};
</script>
